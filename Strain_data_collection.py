...
row_buffer = {}  # timestamp -> {left_knee_ohms, right_knee_ohms}
...
def make_handler(dev):
    def handle(sender, data: bytearray):
        ...
        for i in range(8):
            ...
            if recording.is_set():
                with csv_lock:
                    t_rounded = round(t_sec, 3)  # Round to 3 decimals for merging
                    if t_rounded not in row_buffer:
                        row_buffer[t_rounded] = {'timestamp_s': t_rounded}
                    if dev == DEVICES["L"]:
                        row_buffer[t_rounded]['left_knee_ohms'] = ohms
                    else:
                        row_buffer[t_rounded]['right_knee_ohms'] = ohms
                    csv_writer.writerow(row_buffer[t_rounded])
...
pause_ax = fig.add_axes([0.81, 0.01, 0.08, 0.05])
pause_btn = plt.Button(pause_ax, "Pause", color="#e8e8e8", hovercolor="#d0d0d0")

record_ax = fig.add_axes([0.91, 0.01, 0.08, 0.05])
record_btn = plt.Button(record_ax, "Record", color="#ffcccc", hovercolor="#ff9999")
...
def toggle_record(event):
    ...
    if recording.is_set():
        ...
        if csv_file:
            csv_file.close()
        row_buffer.clear()
        record_btn.label.set_text("Record")
        record_btn.color = "#ffcccc"
        print("Recording stopped, CSV saved.")
    else:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_file = open(f"strain_data_{timestamp}.csv", "w", newline="")
        csv_writer = csv.DictWriter(csv_file, fieldnames=['timestamp_s', 'left_knee_ohms', 'right_knee_ohms'])
        csv_writer.writeheader()
        recording.set()
        record_btn.label.set_text("Stop")
        record_btn.color = "#ff6666"
        print(f"Recording started: strain_data_{timestamp}.csv")
    fig.canvas.draw_idle()


pause_btn.on_clicked(toggle_pause)
record_btn.on_clicked(toggle_record)
...
def update(frame):
    ...
    rec_str = "  [RECORDING]" if recording.is_set() else ""
    title_txt.set_text("  ·  ".join(statuses) + pause_str + rec_str)
